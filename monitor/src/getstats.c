...
get_cpu_util(json_object *jobj)
{
    ...
    static char ret[BUFFERSIZE + 1];
    char *tmp;
    unsigned long int u_ticks = 0, n_ticks = 0, s_ticks = 0, i_ticks = 0, t_ticks = 0;
    static unsigned long int u_ticks_o = 0, n_ticks_o = 0, s_ticks_o = 0, i_ticks_o = 0;
    unsigned int result;
...
get_mem_stats(json_object *jobj)
{
    ...
    while (!feof(fd)) {
        ...
        if (!strncmp(buffer, "MemTotal:", 9)) {
            ...
            while (isspace(*tmp))
                tmp++;          // Increment pointer position til real data
...
/*
  if ( receive_o ) {
    t_receive = (unsigned long)((((receive - receive_o) / REFRESH) + 512) / 1024) - 1;
  }
  if ( transmit_o ) {
    t_transmit = (unsigned long)((((transmit - transmit_o)/ REFRESH) + 512) / 1024) - 1;
  }
*/
    receive_o = receive;
    transmit_o = transmit;
